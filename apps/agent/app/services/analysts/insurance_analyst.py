...
def analyze_insurance(
    *,
    ...
    llm = generate_insurance_analysis(text=text, scenario_type=scenario_type, facts=facts, evidence=evidence)
    if llm:
        return guard_insurance_output(llm, evidence)
    claim_type = ["대물"]
    if facts.get("injury"):
        claim_type.append("대인")
    return guard_insurance_output({
