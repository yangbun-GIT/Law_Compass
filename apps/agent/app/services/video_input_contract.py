...
def normalize_video_input_contract(
    ...
    *,
    ...
    fallback_observation = _frame_rich_zero_observation_fallback(meta, nested, technical, observations)
    ...
    analysis_recovery = _analysis_recovery_plan(meta, nested, technical, accepted, uncertain, ignored, supporting)
    ...
    return {
        ...
        "observation_quality_summary": _quality_summary(accepted, uncertain, ignored, supporting, confirmation_candidates, confirmation_groups, analysis_recovery),
        "warnings": warnings,
...
def _frame_rich_zero_observation_fallback(
    meta: dict[str, Any],
    nested: dict[str, Any],
    technical: dict[str, Any],
    ...
    if frame_count < FRAME_RICH_RECOVERY_MIN_FRAMES:
        return None
    if not _vision_analysis_attempted(meta, nested):
        return None
    frame_refs = _fallback_frame_refs(meta, nested)
    if not frame_refs:
        return None
    return {
...
def _fallback_frame_refs(meta: dict[str, Any], nested: dict[str, Any]) -> list[str]:
    refs: list[str] = []
    for source in (nested, meta):
        if not isinstance(source, dict):
            continue
    ...
    if not refs:
        refs = [str(item).rsplit("/", 1)[-1].rsplit("\\", 1)[-1] for item in (_frame_list(nested) or _frame_list(meta)) if item]
    unique_refs = list(dict.fromkeys(refs))
    return unique_refs[:6]


def _vision_analysis_attempted(meta: dict[str, Any], nested: dict[str, Any]) -> bool:
    for source in (nested, meta):
        if not isinstance(source, dict):
            continue
...
def _analysis_recovery_plan(
    ...
    openai = _analysis_payload(meta, nested, "openai_frame_analysis")
    yolo = _analysis_payload(meta, nested, "yolo_frame_analysis")
...
def _analysis_payload(meta: dict[str, Any], nested: dict[str, Any], key: str) -> dict[str, Any] | None:
    for source in (nested, meta):
        if not isinstance(source, dict):
            continue
        payload = source.get(key)
