...
const kniaRatioSource = [kniaChartView, kniaRankingView, kniaFaultRatioBar, styles].join("\n");
...
const kniaColorLeaks = forbiddenKniaColors.filter((token) => kniaRatioSource.includes(token));
...
const guidedFlowContracts = [
  ...
  "2 대분류·확인질문",
  "3 결과",
  "영상이나 사고 설명을 먼저 넣어 주세요",
  "대분류 및 확인질문으로 계속",
  "영상/설명만으로 바로 분석",
  "대분류 및 질문을 건너뛰고 분석을 진행하시겠습니까?",
  "사고 대분류를 먼저 선택해 주세요",
  "직접 충돌한 상대를 기준으로 선택합니다",
  "대분류별 확인 질문",
  "선택한 대분류에 맞춰 과실비율에 영향을 줄 수 있는 점만 확인할게요",
  "잘 모르겠어요",
  "이대로 분석하기",
  "고급 진단 보기",
  "영상 확인 중",
...
const instantVideoCaseContracts = [
  "영상이나 설명부터 바로 시작합니다",
  "createImmediately",
  "router.replace(`/cases/${data.case.id}?start=input`)",
  "DEFAULT_TITLE",
