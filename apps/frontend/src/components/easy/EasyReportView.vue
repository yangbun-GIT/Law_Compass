<template>
  <section class="easy-report" v-if="safeReport">
    <section v-if="isUserFriendlyMode" class="user-report">
      ...
      <section class="card easy-card simple-section corner-flourish">
        ...
        <div class="easy-ratio-row">
          <div>
            <p>내 과실</p>
            <span>{{ ratioText(simpleFaultRatio.my ?? simpleFaultRatio.my_percent ?? simpleFaultRatio.my_fault) }}</span>
          </div>
          <div>
            <p>상대 과실</p>
            <span class="accent">{{ ratioText(simpleFaultRatio.other ?? simpleFaultRatio.other_percent ?? simpleFaultRatio.opponent_fault) }}</span>
          </div>
        </div>
        <p class="easy-summary">
          {{ text(simpleFaultRatio.basis || simpleFaultRatio.summary || safeReport?.fault_ratio_summary, "입력한 사고 사실과 KNIA 기준을 함께 검토한 참고용 산정입니다.") }}
        </p>
      ...
      <section class="card easy-card simple-section corner-flourish">
        ...
        <div v-if="simpleKniaEvidence" class="basis-card">
          <p class="accent-text">
            {{ text(simpleKniaEvidence.chart_no || simpleKniaEvidence.subchart_no, "KNIA 기준 확인 중") }}
            <span v-if="simpleKniaEvidence.title || simpleKniaEvidence.chart_title">
              · {{ text(simpleKniaEvidence.title || simpleKniaEvidence.chart_title) }}
            </span>
          </p>
          <p v-if="menuPathText(simpleKniaEvidence)">
            {{ menuPathText(simpleKniaEvidence) }}
          </p>
          ...
          >
            {{ text(simpleKniaEvidence.button_label, simpleKniaEvidence.video_url ? "KNIA 관련 영상 보기" : "KNIA 원문 기준 보기") }}
          </a>
          ...
          <p class="kv">
            {{ text(simpleKniaEvidence.source_notice, "영상 파일은 LawCompass 서버에 저장하지 않고, 과실비율정보포털 원본 링크로만 제공합니다.") }}
          </p>
        </div>
    ...
    <section v-else class="expert-report expert-mode">
      ...
      <article v-if="safeReport.knia_fault_adjustment_card" class="card easy-card wide-card knia-adjustment-card">
        ...
        <div class="chips">
          <span class="chip selected" v-if="safeReport.knia_fault_adjustment_card.base_fault">
            기준 A{{ safeReport.knia_fault_adjustment_card.base_fault.A }} : B{{ safeReport.knia_fault_adjustment_card.base_fault.B }}
          </span>
        ...
        <div class="basis-grid">
          ...
          <section class="basis-card">
            ...
            <p v-else class="kv">추가 확인이 필요한 가감요소가 별도로 표시되지 않았습니다.</p>
          </section>
        </div>
      ...
      <article class="card easy-card">
        <h2>{{ text(safeReport.legal_explanation?.title, "법률 확인사항") }}</h2>
      ...
      <article class="card easy-card wide-card">
        ...
        <p v-if="!basisCards.length" class="soft-warning">현재 표시할 수 있는 법률 근거가 부족합니다. 법률 KB 적재 상태와 외부 API 권한을 확인한 뒤 다시 분석해 주세요.</p>
      ...
      <article v-if="visibleKniaBasisCards.length" class="card easy-card wide-card">
        ...
        <div class="basis-grid">
          <div class="basis-card" v-for="card in visibleKniaBasisCards" :key="`${card.chart_no}-${card.title}`">
            ...
            <h3>{{ text(card.title, "KNIA 참고 기준") }}</h3>
...
function text(value: unknown, fallback = "") { return sanitizeDisplayText(value, fallback); }
function ratioText(value: unknown) { return sanitizeDisplayText(value, "확인 필요"); }
...
function menuPathText(card: any) {
  return Array.isArray(card?.menu_path) ? card.menu_path.map(text).filter(Boolean).join(" > ") : "";
}
...
function isAllowedKniaCard(card: any) {
  ...
  if (party.includes("차대사람") || party.includes("car_vs_person")) return chartNo.startsWith("보");
  if (party.includes("차대자전거") || party.includes("bicycle")) return chartNo.startsWith("거");
  if (party.includes("차대차") || party.includes("car_vs_car")) return chartNo.startsWith("차");
