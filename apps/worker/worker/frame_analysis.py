...
def _openai_frame_analysis_prompt(
    ...
    return (
        ...
        "The ego camera may be mounted on a motorcycle or other two-wheeler; identify ego_vehicle_type when the cockpit, handlebar, mirror, riding posture, or mounting viewpoint supports it. "
        "School-zone and speed-limit facts must come only from visible signs, road markings, reliable overlay text, or repeated scene context. Do not infer actual driving speed from camera motion. Use speed_limit_kmh only for posted/marked limit values such as 30, not for estimated vehicle speed. "
        "If captions, subtitles, or dashboard overlays mention a place or condition, emit overlay_text_hint as a hint only; do not use overlay text alone to prove child status, speed violation, fault, or signal violation. "
        "A cyclist is a bicycle target, not a pedestrian. A possible child cyclist should be represented as direct_collision_partner_type=bicycle plus child_candidate=true only when visual size/appearance suggests it; otherwise leave child status for follow-up confirmation. "
        "For an oncoming bicycle or cyclist crossing/entering the ego path, emit oncoming_bicycle_present=true, opposing_direction_actor_type=bicycle, and collision_point_location when supported by the frames. "
        ...
        "crosswalk_nearby, pedestrian_visible, pedestrian_signal, school_zone, ego_vehicle_type, "
        "speed_limit_kmh, road_marking_school_zone_visible, speed_limit_sign_visible, "
        "oncoming_traffic_present, oncoming_bicycle_present, opposing_direction_actor_type, "
        "vulnerable_road_user, child_candidate, overlay_text_hint, damage_level, "
