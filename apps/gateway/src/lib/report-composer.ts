...
function safeKniaUrl(value: any) {
  const text = String(value ?? "").trim();
  if (!text || /\s/.test(text) || !/^https?:\/\//i.test(text)) return "";
  try {
    const url = new URL(text);
    const host = url.hostname.toLowerCase();
    if (!KNIA_ALLOWED_LINK_HOSTS.has(host)) return "";
...
function safeKniaThumbnailUrl(value: any) {
...
function composeKniaLinkCards(result: AnyRecord = {}, report: AnyRecord = {}): AnyRecord {
...
function removeDuplicateKniaRelatedVideo(report: AnyRecord = {}, canonicalCard?: AnyRecord) {
