...
- Agent는 영상 원본을 직접 분석하지 않고, 전처리 요약, 케이스 설명, 구조화 입력, 선택 키워드를 기반으로 분석

현재 구조는 “영상 직접 AI 비전 분석”이 아니라 “영상 metadata/frame preprocessing + 텍스트/구조화 입력 기반 분석”이다.
...
- 단기: 현재 ffmpeg/ffprobe 기반 전처리 유지
