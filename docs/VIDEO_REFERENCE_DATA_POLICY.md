...
2. 공개 플랫폼 영상은 기본적으로 링크와 수동 요약만 manifest에 기록한다.
3. 공개 영상 원본을 다운로드하거나 재배포하는 흐름은 기본 개발 경로로 두지 않는다.
...
- 허용: URL, 제목, 공개 설명의 짧은 수동 요약, 전문가 의견의 짧은 요약, 실제 결과가 공개된 경우 결과 여부, 평가 focus 기록
