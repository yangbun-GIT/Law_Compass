...
Agent 계층만 빠르게 확인하려면 아래 명령을 사용합니다. 이 명령은 Agent Python 컴파일, `/internal/v1/*` 라우트 계약, 대표 사고 판단 회귀 시나리오를 확인합니다.
...
OpenAI 프레임 분석까지 필수로 검증하려면 worker를 `ENABLE_OPENAI_FRAME_ANALYSIS=1`로 재기동한 뒤 아래처럼 실행합니다.
