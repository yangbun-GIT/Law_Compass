...
- GitHub 목록에서 바로 보이는 커밋 메시지와 PR 제목은 팀원이 빠르게 파악할 수 있도록 한국어로 작성합니다. PR 본문은 토큰 효율을 우선해 간결하게 작성하되, 검증 결과와 남은 리스크는 확인 가능하게 남깁니다.
- 작업 단위가 끝나면 백업과 팀원 동기화를 위해 검증 가능한 상태로 커밋하고 원격 저장소에 푸시합니다. 사용자가 PR을 생략하고 직접 `main`에 반영하라고 명시한 경우에는 최신 `main` 확인, 커밋 전 검증, 민감정보/대용량 산출물 제외를 확인한 뒤 직접 푸시할 수 있습니다.
...
PR 제목은 한국어로 작성합니다. PR 본문은 토큰 효율을 우선해 간결하게 적고, 아래 항목을 확인 가능하게 남깁니다.
...
코드 리뷰, PR 리뷰, 팀원 커밋 점검, 교수님 리뷰 대비 점검을 요청받으면 먼저 `docs/CODE_REVIEW_PROMPT.md`를 읽고 findings 중심으로 검토합니다.
