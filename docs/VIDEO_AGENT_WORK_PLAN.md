...
- 상태: 완료. `scripts/aihub597_labels_to_manifest.py`에 `--balanced` 샘플링을 추가해 AI-Hub 597 라벨 200건을 차대차, 차대보행자, 차대이륜차, 차대자전거 각 50건으로 균형 추출했다. `scripts/evaluate_aihub597_label_reference.py`를 추가해 라벨 manifest의 사고대상 분포, 오염 방지 guard, expected context coverage, 원천 영상 검증 필요 여부를 평가한다. 2026-05-29 기준 preflight와 label reference eval 모두 `passed`다. OpenAI+YOLO 실제 출력과의 비교는 원천 영상 소량 다운로드가 필요하므로 다음 단계 입력으로 남긴다.
...
- 상태: 완료. `scripts/aihub597_labels_to_manifest.py --balanced --per-target 50`로 200건 균형 manifest를 생성했고, `scripts/validate_reference_case_manifest.py`와 `scripts/evaluate_aihub597_label_reference.py`를 통과했다. 정적 label reference 평가 결과는 `vehicle=50`, `pedestrian=50`, `motorcycle=50`, `bicycle=50`, `known_direct_target_rate=1.0`, `pollution_guard_coverage=1.0`, `expected_context_coverage=1.0`, `failure_axes=[]`, status `passed`다. 이 단계에서는 라벨만으로 가능한 대규모 reference 준비와 실패 축 분류 기준을 완료했고, OpenAI+YOLO 출력 직접 비교는 원천 영상 소량 다운로드가 필요하다고 명확히 분리했다.
