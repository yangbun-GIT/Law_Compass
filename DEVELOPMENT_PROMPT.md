...
<system_prompt>
  ...
  <development_guidelines>
    ...
    <guideline name="bounded_self_review_loop">
      ...
      문제를 발견하면 같은 작업 안에서 재작업하되, 일반 작업은 최대 1회, Agent 판단·영상 사실 추출·인증·DB·스토리지처럼 정확도나 데이터 무결성에 큰 영향을 주는 작업은 최대 2회까지만 반복합니다.
      2회 점검 후에도 해결되지 않는 문제는 무리하게 루프를 늘리지 말고 남은 리스크와 차단 사유를 명확히 기록합니다.
      ...
      외부 LLM/API/영상 분석 호출이 필요한 자기검토는 비용, timeout, token cap, sample 수를 먼저 제한하고, 가능하면 deterministic fixture나 로컬 테스트로 대체합니다.
    </guideline>
