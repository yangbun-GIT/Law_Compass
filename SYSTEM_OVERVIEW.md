...
### 2026-05-23 변호사 의견 reference set 실제 OpenAI 배치 점검

`logs/video_accuracy/lawyer_reference_manifest.json`의 사고 영상 1~5번을 실제 OpenAI 프레임 분석 모드로 배치 점검했다. 전체 배치에서는 5개 중 4개가 통과했고, 사고4는 분석 자체가 아니라 `missing_info.priority_items`에 내부 field key가 노출될 수 있는 표시 안전성 검증에서 실패했다. Gateway 표시 계층을 수정한 뒤 사고4 단일 재시도는 통과했다. 따라서 현재 판정은 “영상 처리/Agent 연결은 5개 샘플 모두 실행 가능하나, 사고별 기대 과실범위와 판례·보험 근거 일치도 평가는 다음 단계에서 별도 기준을 세워야 함”이다.

| 샘플 | OpenAI 프레임 분석 관찰 요약 | 후속 판단 |
| --- | --- | --- |
| 사고1 | `stopped`, `opponent_behavior`, `collision_direction` 후보가 생성됐으나 모두 보류됐다. | 복합 충돌 사고라 관찰값을 바로 fact로 승격하지 않는 정책은 적절하다. 중앙선 침범 사유/2차 추돌 분리 질문이 필요하다. |
| 사고2 | `crosswalk_nearby`가 반영됐다. | 핵심 쟁점은 신호 전환 시점인데 영상 관찰만으로는 부족하므로 CCTV/신호체계 확인 질문이 우선이다. |
| 사고3 | 정차, 횡단보도, 우회전 방향지시등 등 관찰값이 생성됐고 일부 반영됐다. | 앞차 정지 사유와 후방 추돌 기본책임을 함께 다루는 판례/KNIA 근거 평가가 필요하다. |
| 사고4 | 재시도에서 `stopped=false`, `impact_direction=rear`, `opponent_behavior=stopped` 등이 생성됐지만 모두 보류됐다. | 무등화 정차 차량, 속도, 회피 가능성 감정이 핵심이므로 영상 관찰만으로 결론을 내리지 않고 형사/민사 쟁점을 분리해야 한다. |
| 사고5 | 정차, 급정거, 후방 고속버스 추돌 후보가 생성됐고 일부 반영/확인/충돌로 나뉘었다. | 자전거 비접촉 유발과 후방 버스 책임을 분리하는 추가 fact schema와 근거 검색 기준이 필요하다. |

표시 안전성 수정 범위는 `apps/gateway/src/lib/report-composer.ts`와 `apps/gateway/test/report-composer.test.ts`다. 지원하지 않는 내부 required input field는 공개 보완 질문에서 제외하고, 지원 field의 label은 항상 `videoFactLabel()` 기반 한국어 라벨로 매핑한다. 이 변경은 DB schema, Redis key, storage path, 외부 API 계약을 변경하지 않는다.
...
로컬 평가 결과는 `logs/video_accuracy/reference_guidance_eval.json`에 저장한다. `logs/`는 Git에서 제외되므로 실제 영상 경로, 케이스 입력, 전문가 참고 의견 기반 측정 결과가 저장소에 올라가지 않는다. 2026-05-23 기준 사고 1~5번은 pipeline 기준 모두 통과했고, 사고 3과 사고 5는 사용자 입력과 영상 관찰값 충돌을 먼저 해소해야 하는 상태로 분류됐다. 나머지 사고 1, 2, 4는 법령/KNIA/판례/보험 근거 대조 단계로 넘어갈 수 있는 상태로 분류됐다.
