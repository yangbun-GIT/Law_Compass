...
| 문서 | 역할 |
| --- | --- |
| `docs/GITHUB_COLLABORATION_WORKFLOW.md` | 브랜치 이름 규칙, 작업 시작 절차, 커밋 전 확인, 검증 기준, PR 작성/리뷰/병합, 충돌 처리, 문서 업데이트 기준 |

협업 방식은 `main`을 실행 가능한 기준 브랜치로 유지하고, 모든 작업을 목적별 브랜치에서 진행한 뒤 PR 리뷰 후 병합하는 GitHub Flow를 기본으로 한다. `.env`, API key, 사용자 비밀번호, 영상 원본, `storage/`, `logs/`, AI Hub 원본 데이터, YOLO 모델 가중치는 계속 Git에 올리지 않는다.
